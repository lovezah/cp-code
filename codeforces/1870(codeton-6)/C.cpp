...
void solve() {
    read(n, k);
    F(i, 1, n+1) read(a[i].f), a[i].s = i+1;
    sort(a+1, a+n+1);
    F(i, 1, k+1) mn[i]=2e9, mx[i]=0;
    suf[n+1][0] = 0;
    FF(i, 1, n+1) suf[i][0] = a[i].s, ckmax(suf[i][0], suf[i+1][0]);
    suf[n+1][1] = 2e9;
    FF(i, 1, n+1) suf[i][1] = a[i].s, ckmin(suf[i][1], suf[i+1][1]);
    F(i, 1, n+1) {
        ckmax(mx[a[i].f], suf[i][0]);
        ckmin(mn[a[i].f], suf[i][1]);
    }
    F(i, 1, k+1) {
        ll ans = (mx[i] - mn[i] + 1) * 2;
        if (mn[i] == 2e9) ans = 0;
        write(ans, ' ');
    }
    print();
